class Solution(object):
    def maxProduct(self, nums):
        ...
        for i in xrange(1, len(nums)):
            if nums[i] < 0:
                mintemp, maxtemp = maxtemp,mintemp  # swap min and max temp if integer is negative
                
            maxtemp = max(nums[i], nums[i] * maxtemp) # either restart from this number or let this num multiply previous maximum
...
for every number, consider either restarting from this one or multipy the previous max/min
O(n) time, O(1) space
'''
